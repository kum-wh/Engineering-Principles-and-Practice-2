...
void handleUltrasonic(TPacket *packet) {
	if (packet->params[0] == 0) {
		...
		printf("Distance to obstacle: %d mm\n",packet->params[1]);
	}
}
...
void handleColor(TPacket *packet) {
	if (packet->params[0] > packet->params[1]) {
		printf("Red \n");
	} else if (packet->params[0] < packet->params[1]) {
		printf("Green \n");
	} else {
...
int main()
{
	...
	{
		char ch;
		printf("Command (w=forward, s=reverse, a=turn left, d=turn right, s=stop, c=colour, g=get stats q=exit)\n");
		scanf("%c", &ch);
